...
var (
	cfgFile string
	globalOpts = config.GlobalOptions{Temperature: 0.2}
)
...
func init() {
	...
	flags.StringVar(&globalOpts.Model, "model", globalOpts.Model, "Model identifier (e.g. gpt-4o-mini)")
	flags.StringVar(&globalOpts.Provider, "provider", globalOpts.Provider, "Model provider (openai|azure|bedrock|ollama|vllm|http)")
