...
func (r *Registry) Snapshot() []ClientInfo {
	...
	for _, client := range r.clients {
		...
		if client.Manifest != nil {
			...
			if len(client.Manifest.Capabilities) > 0 {
				info.ManifestCapabilities = append([]string(nil), client.Manifest.Capabilities...)
				sort.Strings(info.ManifestCapabilities)
			}
			if client.Manifest.Transport != nil {
				if kind, ok := client.Manifest.Transport["type"].(string); ok {
					info.ManifestTransportType = kind
				}
			}
		}
